...
def generar_hash
 ...
 def add_fechas_contexto(search_string, my_arr)
  ...
  search_string = search_string.encode("UTF-8", :invalid=>:replace, :replace=>"?").encode('UTF-8')
  my_arr.each do |f|
    clean_f = clean_regex f[1]
    f[1] = f[1] + " -> "+ search_string.scan(/.{50}#{clean_f}.{50}/)[0] if !search_string.scan(/.{50}#{clean_f}.{50}/)[0].nil?
  end
  return my_arr
 end
 ...
 def scan_my_str(my_s, st)
	 ...
	 my_s = my_s.encode("UTF-8", :invalid=>:replace, :replace=>"?").encode('UTF-8')
     last_match = 0
     clean_st = clean_regex st
     my_s.scan(/#{clean_st}/).each_with_index do |a,b|
