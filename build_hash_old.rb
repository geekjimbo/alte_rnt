...
def generar_hash
 ...
 def get_the_hash(my_arr)
   ...
   debugger
   my_arr.each do |arr|
    file_name = arr.inspect.scan(/:.*_\d{3}.txt/)[0].gsub(/:/,"")
    f = File.open("./"+file_name)
    my_str = ''
    my_str = f.read
    r = extraer my_str
 ...
 def get_numero_resolucion(search_string)
  ...
  my_hash = convert_num_resolucion_to_hash context_resultado 
  return my_hash
 end
 ...
 def add_contexto(search_string, my_str)
  my_result = []
  my_str.each do |x|
      my_search = search_string.scan(/.{50}#{x}.{50}/)[0]
      my_search = search_string.scan(/.{50}#{x}.{25}/)[0] if my_search.nil?
      my_search = search_string.scan(/.{50}#{x}/)[0] if my_search.nil?
      my_search = search_string.scan(/.{30}#{x}/)[0] if my_search.nil?
      my_search = search_string.scan(/#{x}/)[0] if my_search.nil?
      my_search = "" if my_search.nil?
      my_result << x+"  ->  "+ my_search if x.split.size > 1
  end
  return my_result
 end
